...
| `sample_count` | `N`, the count of qualifying hours (comfort-score.md Step 2) — this counts hours that passed the minimum-traffic filter, not raw sensor events. The traffic count itself (`T_h`) is sourced from `hourly_segment_features.trip_count`; whether the Gold job joins that table or `hourly_comfort_score` grows its own traffic-count column is **open** — see OQ-039 |
