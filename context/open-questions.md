...
| OQ-002 | Which S3 file/table format and local development access pattern will be used? | The prototype writes Kafka envelopes as Parquet through Spark S3A and partitions them by UTC `event_date/hour`; local paths use the same Parquet sink. A production table format/catalog policy remains undecided (ADR-0009 only adds Parquet-preserving compaction). Until it is settled, Silver hour partitions are committed by staging + rename and accept EMRFS's non-atomic rename (ADR-0011); adopting a table format such as Iceberg would replace that mechanism, so revisit ADR-0011 together with this question. | Prototype implemented 2026-08-22; production policy remains open |
