...
class StreamConfig:
    ...
    def from_env(cls, env: Mapping[str, str] | None = None) -> StreamConfig:
        ...
        # 아래는 전부 `.get(KEY) or 기본값` 형태다 — docker compose는 호스트 .env에 값이
        # 없으면 빈 문자열을 주입하므로(#409), 빈 값을 미설정과 같게 봐야 한다. `.get(KEY,
        # 기본값)`은 키가 없을 때만 동작해 빈 값에서는 int("")로 기동이 죽었다(#592).
        # 반드시 int/float 변환 *전* 문자열 단계에서 걸러야 한다. "0"은 truthy라 명시적
        # 0 설정(트리거 하한 끄기, 복구 배치 상한 없음)이 기본값으로 뒤바뀌지 않는다.
        return cls(
            # sensor-producer가 이미 쓰는 환경변수 이름과 동일하게 맞춰 두 서비스가
            # 별도 변환 없이 같은 Kafka 엔드포인트/토픽을 공유하게 한다.
            bootstrap_servers=source.get("KAFKA_BOOTSTRAP_SERVERS") or "localhost:9092",
            topic=source.get("KAFKA_SENSOR_TOPIC") or "sensor-events",
            trigger_interval_seconds=float(source.get("STREAM_TRIGGER_INTERVAL_SECONDS") or "30"),
            bronze_output_path=(
                source.get("STREAM_BRONZE_OUTPUT_PATH") or "data/local-lake/bronze/sensor-events"
            ),
            bronze_checkpoint_location=(
                source.get("STREAM_BRONZE_CHECKPOINT_LOCATION")
                or "checkpoints/bronze-sensor-events"
            ),
            # 체크포인트가 없을 때만 적용되며, 체크포인트가 있으면 이 값과 무관하게
            # Spark가 체크포인트 기준으로 재개한다 (재시작 시 재개 조건 충족).
            starting_offsets=source.get("KAFKA_STARTING_OFFSETS") or "earliest",
            ...
            # 기다리지 않는다.
            min_offsets_per_trigger=int(source.get("STREAM_MIN_OFFSETS_PER_TRIGGER") or "600000"),
            ...
            # 위 상한과 짝이다(#482).
            driver_memory=source.get("STREAM_DRIVER_MEMORY") or "4g",
        )
