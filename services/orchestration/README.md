...
운영 배포 전에 GitHub repository variable을 다음처럼 준비한다.

| 변수 | 값 | 설명 |
| --- | --- | --- |
| `SENSOR_EVENTS_COMPACTION_ROOT_URI` | `s3://<data-lake>/bronze/sensor-events` | 필수, 다른 경로나 로컬 경로면 배포 실패 |
| `SENSOR_EVENTS_COMPACTION_ENABLED` | `true` | 배포가 DAG의 pause 상태를 이 값에 맞춤 |
| `SENSOR_EVENTS_COMPACTION_MAX_GROUPS_PER_RUN` | 최초 `1` | 최초 S3 canary가 처리할 시간 파티션 수 |

첫 실행 후에는 압축 전후 row 수, `compacted-*` 생성, 원본 삭제,
